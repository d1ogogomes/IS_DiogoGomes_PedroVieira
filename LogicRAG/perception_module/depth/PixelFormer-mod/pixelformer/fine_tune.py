...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.checkpoint_path != '':
        ...
        if os.path.isfile(args.pretrained_model_path):
            ...
            print("== No model found at '{}'".format(args.pretrained_model_path))
        model_just_loaded = True
        del checkpoint
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            # print(depth_est.shape, depth_gt.shape, torch.unique(depth_est), torch.unique(depth_gt), mask.to(torch.bool), loss)
            loss.backward()
...
def main():
    if args.mode != 'train':
        print('fine_tune.py is only for training.')
        return -1
